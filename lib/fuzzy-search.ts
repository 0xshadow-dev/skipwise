 ...
 */
function calculateFuzzyScore(pattern: string, text: string, options: {
  caseSensitive?: boolean
  ...
  if (normalizedPattern === normalizedText) {
    return { score: 1, indices: Array.from({ length: pattern.length }, (_, i) => i) }
  }
  ...
  if (substringIndex !== -1) {
    const indices = Array.from({ length: patternLength }, (_, i) => substringIndex + i)
...
export class FuzzySearcher<T> {
  ...
  search(query: string): SearchResult<T>[] {
    ...
    for (const item of this.items) {
      ...
      for (const key of this.options.keys) {
        ...
        try {
          if (key.getFn) {
            ...
            value = String((item as any)[key.name] || '')
          }
        } catch (error) {
          ...
          if (normalizedText.includes(trimmedQuery)) {
            ...
            highlights.push({
              ...
              highlights: [{
                ...
                text: textValue.slice(startIndex, startIndex + trimmedQuery.length)
              }]
            })
          } else {
            // Try fuzzy matching
            const fuzzyResult = calculateFuzzyScore(trimmedQuery, normalizedText, {
              caseSensitive: false,
              penalizeDistance: true,
              bonusConsecutive: true
            })
            
            if (fuzzyResult.score > 0.3) { // Lower threshold for fuzzy matches
      ...
      if (matchFound) {
        // Normalize score
        const normalizedScore = Math.min(1, totalScore / this.options.keys.length)
   ...
   */
  searchWithTypoTolerance(query: string, maxTypos: number = 2): SearchResult<T>[] {
    ...
    for (const item of this.items) {
      ...
      for (const key of this.options.keys) {
        const weight = key.weight || 1
        const value = key.getFn ? key.getFn(item) : (item as any)[key.name]
        ...
        for (const textValue of textValues) {
          ...
          for (const word of words) {
            const editDistance = calculateEditDistance(query.toLowerCase(), word)
            
            if (editDistance <= maxTypos && word.length > 2) {
              const typoScore = Math.max(0, 1 - (editDistance / Math.max(query.length, word.length)))
      ...
      if (bestScore >= this.options.threshold * 0.5) { // Lower threshold for typo matches
        typoResults.push({
          item,
          score: bestScore,
          matches: this.options.includeMatches ? matches : [],
    ...
    return typoResults.sort((a, b) => b.score - a.score).slice(0, this.options.maxResults)
  }
}
